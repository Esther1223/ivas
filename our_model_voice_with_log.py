...
def save_prediction_log(mode, result, source_image_path="", saved_image_path="", fps=None):
    ...
    fieldnames = [
        "time", "mode", "source_image_path", "saved_image_path",
...
def save_judged_image(img, result, mode, source_name=""):
    """把當下用來判斷的照片存下來，並回傳存檔路徑。"""
    global last_saved_image_time

    if not ENABLE_SAVE_IMAGES:
        return ""

    if SAVE_ONLY_WHEN_NOT_STRAIGHT and result["decision"] == "直走":
        return ""

    now = time.time()
    if mode == "camera" and SAVE_IMAGE_INTERVAL_SEC > 0:
        if now - last_saved_image_time < SAVE_IMAGE_INTERVAL_SEC:
            return ""
    ...
    try:
        ...
        final_state = f"L-{result['left_final']}_C-{result['center_final']}_R-{result['right_final']}"

        if source_name:
            base = os.path.splitext(os.path.basename(source_name))[0]
            filename = f"{timestamp}_{mode}_{base}_{safe_decision}_{final_state}.jpg"
        else:
...
def run_camera():
    ...
    try:
        while True:
            ...
            saved_image_path = save_judged_image(img, result, "camera")
            save_prediction_log("camera", result, saved_image_path=saved_image_path, fps=fps)
...
def run_folder(folder_path):
    ...
    for idx, filename in enumerate(image_files, start=1):
        ...
        try:
            ...
            result = predict_one_image(img)
            speak_decision(result["decision"])
            saved_image_path = save_judged_image(img, result, "folder", source_name=path)
            save_prediction_log("folder", result, source_image_path=path, saved_image_path=saved_image_path)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("請選擇模式：")
        print("  camera 模式：python3 our_model_voice.py camera")
        print("  folder 模式：python3 our_model_voice.py folder /home/username/test_images")
        sys.exit()
    ...
    if mode == "camera":
        ...
        if len(sys.argv) < 3:
            print("請輸入圖片資料夾路徑")
            print("例如：python3 our_model_voice.py folder /home/username/test_images")
        else:
